...
{
	public class CreditNoteStatusChangedHandler : IDomainEventHandler<CreditNoteStatusChanged>
    {
	   ...
	   public CreditNoteStatusChangedHandler()
        {
		...
		public void When(CreditNoteStatusChanged @event)
        {
			...
			_eventStoreOperations.SaveEventToStream(@event, metaDataObj, environment + @event.CreditNoteModel.CompanyId + "-CreditNote", typeof(CreditNoteStatusChanged).Name);
        }
    }
