...
{
    public class CreditMemoCreatedHandler : IDomainEventHandler<CreditMemoCreated>
    {
        ...
        public CreditMemoCreatedHandler()
        {
        ...
        public void When(CreditMemoCreated @event)
        {
            ...
            object metaDataObj = new
            {
                ...
                Description = String.Format("A CreditMemo is created by {0} on {1}", @event.CreditMemoModel.UserCreated, @event.CreditMemoModel.CreatedDate)
            };
            _eventStoreOperations.SaveEventToStream(@event, metaDataObj, environment + @event.CreditMemoModel.CompanyId + "-CreditMemo", typeof(CreditMemoCreated).Name);
        }
    }
}
