...
namespace Kendo.DynamicLinq
{
    public static class QueryableExtensions
    {
        ...
        public static async Task<DataSourceResult> ToDataSourceResultAsync<T>(this IQueryable<T> queryable, int take, int skip, IEnumerable<Sort> sort, Filter filter, int recordsCount)
        {
            return await queryable.ToDataSourceResult1Async(take, skip, sort, filter, null, recordsCount);
        }

        public static async Task<DataSourceResult> ToDataSourceResult1Async<T>(this IQueryable<T> queryable, int take, int skip, IEnumerable<Sort> sort, Filter filter, IEnumerable<Aggregator> aggregates, int recordsCount)
        {
            // Filter the data first
            queryable = await Task.Run(() => Filter(queryable, filter));

            // Calculate the total number of records (needed for paging)
            var total = recordsCount;

            // Calculate the aggregates
            var aggregate = await Task.Run(() => Aggregate(queryable, aggregates));

            // Sort the data
            queryable = await Task.Run(() => Sort(queryable, sort));

            // Finally page the data
            if (take > 0)
            {
                queryable = await Task.Run(() => Page(queryable, take, skip));
            }
